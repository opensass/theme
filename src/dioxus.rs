...
use web_sys::{HtmlElement, MediaQueryList, js_sys::Date, wasm_bindgen::JsCast};
...
pub fn ThemeProvider(props: ThemeProviderProps) -> Element {
    let val = match props.storage_type {
        StorageType::LocalStorage => web_sys::window()
            .unwrap()
            .local_storage()
            .unwrap()
            .unwrap()
            .get_item(props.storage_name)
            .unwrap(),
    ...
    use_effect(move || {
        ...
        let on_storage: Closure<dyn FnMut(web_sys::StorageEvent)> =
            Closure::wrap(Box::new(move |_event| {
                let val: Option<Theme> = match storage_type {
                    StorageType::LocalStorage => LocalStorage::get(storage_name).ok().flatten(),
                    StorageType::SessionStorage => SessionStorage::get(storage_name).ok().flatten(),
                };

                if let Some(t) = val.clone() {
                    theme.set(t.clone());
